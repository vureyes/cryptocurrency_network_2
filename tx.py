...
def generate_coin_tx(receiver_address, amount=10000):
    new_input = TxIn(b'\x00' * 32, 0xffffffff)
    script_pubkey = p2pkh_script_from_address(receiver_address)
    new_output = TxOut(amount, script_pubkey)
    new_tx = Tx(2, [new_input], [new_output], 0, True)
    return new_tx.serialize()


def create_new_tx(sender_address, tx_hash, receiver_address, index=0, amount=10000):
    new_input = TxIn(bytes.fromhex(tx_hash), index)
    script_pubkey = p2pkh_script_from_address(receiver_address)
    new_output = TxOut(amount, script_pubkey)
    new_tx = Tx(2, [new_input], [new_output], 0, True)
    return new_tx.serialize()
